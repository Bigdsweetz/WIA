$(document).ready(function() {
			...
			(new TimelineLite({onComplete:initScrollAnimations}))
				...
				.from( $('#title-line1 span'), .4, {delay: 1, css:{right:'2000px'}, ease:Back.easeOut})
				//slides in from the bottom
				.from( $('#title-line2'), .4, {css:{top:'2000px',opacity:'0'}, ease:Expo.easeOut})
				.append([
					//this is for each individual letter
					TweenMax.from( $('#title-line3 .char1'), .25+Math.random(), {css:{top: '-200px', right:'2000px'}, ease:Elastic.easeOut}),
					TweenMax.from( $('#title-line3 .char2'), .25+Math.random(), {css:{top: '300px', right:'2000px'}, ease:Elastic.easeOut}),
					TweenMax.from( $('#title-line3 .char3'), .25+Math.random(), {css:{top: '-400px', right:'2000px'}, ease:Elastic.easeOut}),
				])
				//slides in from the right
				.from( $('#title-line4 span'), .4, {delay: 0, css:{left:'2000px'}, ease:Back.easeOut})
			...
			function initScrollAnimations() {
				...
				// title tweens from the bottom up
				...
				controller.addTween(10, TweenMax.to($('#title-line4'), .75, {css:{top: 800}, ease:Quad.easeOut},200));
				

				// explination tweens
				$('#exp h3').lettering().find('span').css('position','relative').each(function() {
					controller.addTween('#exp h3', TweenMax.from( $(this), .25, {delay:Math.random()*.2,css:{left:Math.random()*1200-600,top:Math.random()*600-300,opacity:0}, ease:Expo.easeOut}),200);
				});
				...
				var pinDur = 4000;
				// create animation timeline for pinned element this is for the multi color part
				...
				pinAnimations
					...
					.append([
						...
						TweenMax.to($('#pin-frame-color h2'), .25, {css:{color:'yellow'}})
					])
					.append([
					...
					.append([
						...
						TweenMax.to($('#pin-frame-color h2'), .25, {css:{color:'black'}})
					])
					.append([
						TweenMax.to($('#pin-frame-color'), .25, {css:{backgroundColor:'black'}}),
						TweenMax.to($('#pin-frame-color h2'), .25, {css:{color:'white'}})
					])
					.append([
						TweenMax.to($('#pin-frame-color'), .25, {css:{backgroundColor:'red'}}),
